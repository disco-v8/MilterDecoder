...
pub fn parse_mail(header_fields: &HashMap<String, Vec<String>>, body_field: &str) {
	...
	for (k, vlist) in header_fields { // 各ヘッダ名と値リストをループ
		for v in vlist { // 同じヘッダ名の複数値も対応
	...
	if let Some(msg) = parser.parse(mail_string.as_bytes()) { // パース成功時
		// Fromアドレスを文字列化（複数対応）
		let from = msg.from().map(|addrs| {
			addrs.iter()
				.map(|addr| {
		...
		for (i, part) in msg.parts.iter().enumerate() { // 各パートをインデックス付きでループ
			if part.is_text() { // テキストパートか判定
		...
		// テキストパートごとに本文を出力
		for (idx, _) in text_indices.iter().enumerate() { // テキストパートインデックスをループ
			if let Some(body) = msg.body_text(idx) { // 本文デコード成功時
		...
		for part in msg.parts.iter() { // 各パートを再度ループ
			if !part.is_text() { // テキストパート以外のみ処理
				...
				let fname = part.content_disposition() // Content-Disposition属性を取得
					.and_then(|cd| {
						cd.attributes().unwrap_or(&[]).iter() // 属性リストをイテレート
					...
					.or_else(|| {
						part.content_type() // Content-Type属性も補助的に参照
							.and_then(|ct| {
								ct.attributes().unwrap_or(&[]).iter() // Content-Typeの属性リストをイテレート
